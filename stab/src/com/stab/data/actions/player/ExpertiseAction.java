package com.stab.data.actions.player;

import com.stab.data.StabConstants;
import com.stab.data.animation.DefendAnimation;
import com.stab.model.action.SelfAction;
import com.stab.model.events.common.ConsoleMessage;
import com.stab.model.info.BaseInfo;
import com.stab.model.info.trait.Effect;
import com.stab.model.info.trait.Modifier;
import com.stab.model.request.basic.ActionRequest;

public class ExpertiseAction extends SelfAction {
	
public static final String ID="EXPERTISE";
	
	public ExpertiseAction() {
		this.setName("Expertise");
		this.setResource("actions/ability_warrior_safeguard");
		this.setUsableByAI(false);
	
	}
	
	@Override
	public boolean execute(BaseInfo self) {
	
		Modifier caexpertise= Modifier.createMod(StabConstants.ARMOR, +1);
		Modifier tohitexpertise=Modifier.createMod(StabConstants.TOHIT, -1);
		
		if(self.hasTrait("buffca") == true) {
			return false;
		}
		else {
			
		self.playAnimation(DefendAnimation.ID);
		self.sendMessage(ConsoleMessage.SUCCESS, self.getText()+" activa expertise");
		
		Effect buffcaround = Effect.createRound("buffca", caexpertise, tohitexpertise);
		self.addTrait(buffcaround);
		
		System.out.println("Armadura: " + self.getValue(StabConstants.ARMOR));
		System.out.println("Bono a dar: " + self.getValue(StabConstants.TOHIT));

		return true;
		}
	}
	@Override
	public boolean shouldEndTurn(ActionRequest ar, boolean success) {
		return false;
	}
		
}
